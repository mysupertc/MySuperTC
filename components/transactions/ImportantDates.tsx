              ...
              <div className="space-y-3">
                {datesInGroup.map((dateItem) => {
                  ...
                  return (
                    <Sheet
                      key={dateItem.key}
                      open={openSheetKey === dateItem.key}
                      onOpenChange={(isOpen) => setOpenSheetKey(isOpen ? dateItem.key : null)}
                    >
                        ...
                        >
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-8 h-8 rounded-lg flex items-center justify-center ${statusInfo.color}`}
                              >
...
function DateEditForm({ dateItem, transaction, onUpdate, generateICS }) {
  ...
  const handleDateCalculation = (daysInput) => {
    ...
    if (baseDateStr && daysInput) {
      ...
      const count = Number.parseInt(daysInput)
