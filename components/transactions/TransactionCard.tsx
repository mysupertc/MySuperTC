...
const getStatusInfo = (status) => {
  switch (status) {
    case "active_contingent":
      return { label: "Contingent", color: "bg-yellow-100 text-yellow-800" }
    case "active_noncontingent":
      return { label: "Non-Contingent", color: "bg-blue-100 text-blue-800" }
    case "pending":
      return { label: "Pending", color: "bg-indigo-100 text-indigo-800" }
    case "closed":
      return { label: "Closed", color: "bg-gray-200 text-gray-800" }
    case "cancelled":
      return { label: "Cancelled", color: "bg-red-100 text-red-800" }
    case "seller_in_possession":
      return { label: "Seller in Possession", color: "bg-purple-100 text-purple-800" }
    case "pre_listing":
      return { label: "Pre-Listing", color: "bg-green-100 text-green-800" }
    case "listed":
      return { label: "Listed", color: "bg-green-100 text-green-800" }
    default:
      return { label: status.replace(/_/g, " "), color: "bg-gray-100 text-gray-800" }
  }
}
...
export default function TransactionCard({ transaction }) {
  ...
  return (
    <Link href={createPageUrl("TransactionDetail", `id=${transaction.id}`)} className="group">
      <div className="w-64 bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex-shrink-0">
        ...
        <div className="p-4 h-24 flex flex-col justify-between">
          {daysToCOE !== null && transaction.status !== "closed" && transaction.status !== "cancelled" ? (
            ...
            <p
              className={`text-xs font-semibold uppercase leading-tight ${statusInfo.color.replace("bg-", "text-").replace("100", "700")}`}
            >
              {statusInfo.label}
            </p>
          ) : (
            <p className="text-xs font-semibold text-gray-500 uppercase leading-tight">NEW LISTING</p>
          )}
