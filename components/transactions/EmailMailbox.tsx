...
import { EmailHistory } from "@/lib/api/entities" // Fixed import path
...
const SwipeableEmailItem = ({ email, onSelect, onDelete }) => {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      >
        <div
          className="group flex items-center gap-4 p-4 bg-white rounded-xl hover:bg-gray-50 cursor-pointer border border-gray-100 transition-colors"
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {email.attachments && email.attachments.length > 0 && <Paperclip className="w-3 h-3 text-gray-400" />}
            </div>
...
export default function EmailMailbox({ transactionId, onComposeClick }) {
  ...
  const loadEmails = useCallback(async () => {
    ...
    try {
      ...
      if (transactionEmails.length > 0) {
        ...
        for (const unlinkedEmail of unlinkedEmails) {
          if (unlinkedEmail.gmail_thread_id && transactionThreadIds.has(unlinkedEmail.gmail_thread_id)) {
            console.log(
              `EmailMailbox: Found unlinked email "${unlinkedEmail.subject}" (ID: ${unlinkedEmail.id}) that matches thread for transaction ${transactionId}`,
            )
      ...
      const allEmails = [...transactionEmails, ...linkedUnlinkedEmails].sort(
        (a, b) => new Date(b.sent_at) - new Date(a.sent_at),
      )

      console.log(
        `EmailMailbox: Loaded ${transactionEmails.length} direct emails + ${linkedUnlinkedEmails.length} thread-matched emails = ${allEmails.length} total`,
      )
      setEmails(allEmails)
    } catch (error) {
  ...
  return (
    <Card className="clay-element border-0">
      <CardHeader className="flex flex-row justify-between items-center">
        ...
        <Button size="sm" variant="outline" className="clay-element bg-transparent" onClick={() => onComposeClick()}>
