...
export default function TransactionTimeline({ transaction, taskItems, onUpdate }: any) {
    ...
    switch (showFilter) {
      case "upcoming":
        return timelineItems.filter(
          (item: any) => item.date >= now && item.status !== "completed" && item.status !== "waived",
        )
      case "overdue":
        return timelineItems.filter((item: any) => item.status === "overdue")
      case "all":
      default:
  ...
  const upcomingCount = timelineItems.filter(
    (item: any) => item.date >= new Date() && item.status !== "completed" && item.status !== "waived",
  ).length

  const overdueCount = timelineItems.filter((item: any) => item.status === "overdue").length
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400">
              {showFilter === "upcoming"
                ? "All important dates are either completed or in the past"
                : showFilter === "overdue"
            ...
            {filteredItems.map((item: any, index: number) => {
              ...
              return (
                <div key={item.id} className="relative">
                  {!isLast && <div className="absolute left-6 top-12 bottom-0 w-px bg-gray-200"></div>}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                        <div className="flex-1 min-w-0">
                          ...
                          {item.notes && <p className="text-xs text-gray-600 mt-2 line-clamp-2">{item.notes}</p>}
                        </div>

                        <div className="flex-shrink-0 text-right">
                          <div className={`text-xs ${dateContext.color}`}>{dateContext.text}</div>
                          <div className="text-xs text-gray-400 mt-1">{format(item.date, "MMM d")}</div>
                        </div>
                      </div>
                    </div>
                  </div>
