...
function AddFromMasterList({ onAdd, existingItems, sectionKey }) {
  ...
  const availableDocs = useMemo(() => {
    ...
    return optionalDocuments.filter((doc) => !existingNames.has(doc.document_name) && doc.section === sectionKey)
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="clay-element border-0 justify-between w-[300px] bg-transparent"
          size="sm"
        >
                ...
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    ...
                    const selectedDoc = availableDocs.find(
                      (d) => d.document_name.toLowerCase() === currentValue.toLowerCase(),
                    )
                    if (selectedDoc) {
                      onAdd(selectedDoc)
                    }
...
function ItemRow({ item, columns, onUpdate, onDelete, isNew = false, onAddNew, sectionKey }) {
  ...
  const handleUpdate = useCallback(
    ...
    [isNew, item?.id, onUpdate],
  ) // Depend on item.id for stability of the callback
  ...
  return (
    <tr
      className={`${isCompleted ? "bg-gray-100 opacity-75" : "bg-white hover:bg-gray-50"} ${isOverdue ? "border-l-4 border-red-500" : ""}`}
    >
      {columns.map((col) => (
        <td key={col.key} className="p-3 align-middle relative">
          {col.type === "checkbox" ? (
            ...
            <div className="relative">
              ...
              />
              {isOverdue &&
                col.key === columns.find((c) => c.type === "text")?.key && ( // Display overdue badge on the first text column
...
export default function Checklist({
  ...
  const totalOverdue = useMemo(
    () => items.filter((item) => !completionLogic(item) && item.due_date && isPast(parseISO(item.due_date))).length,
    [items, completionLogic],
  )
        ...
        <thead className="bg-gray-50 rounded-t-2xl">
          <tr>
            {columns.map((col, index) => (
              <th
                key={col.key}
                className={`p-3 text-left font-medium text-gray-700 text-xs ${
