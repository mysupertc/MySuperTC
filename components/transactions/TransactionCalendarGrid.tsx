...
import { TaskItem } from "@/lib/api/entities" // Fixed import path from @/api/entities to @/lib/api/entities
    ...
    <>
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              <div className="px-4 py-2 font-semibold text-gray-700">{format(currentDate, "MMMM yyyy")}</div>
              <Button
                variant="outline"
                size="sm"
          ...
          <div className="grid grid-cols-7 gap-1">
            {calendarDays.map((day) => {
              ...
              return (
                ...
                >
                  <div
                    className={`text-xs font-medium mb-1 ${isDayToday ? "text-blue-700 font-bold" : "text-gray-700"}`}
                  >
                    {format(day, "d")}
                  </div>
                  ...
                  <div className="space-y-1">
                    {dayDates.slice(0, 2).map((dateItem) => (
                      ...
                      >
                        {dateItem.type === "task" ? "📋" : dateItem.type === "disclosure" ? "📄" : "🏠"}{" "}
                        {dateItem.title.split(" ")[0]}
                      </Badge>
                    ))}

                    {dayDates.length > 2 && (
                      <div className="text-xs text-gray-500 text-center">+{dayDates.length - 2}</div>
                    )}
                  </div>
                  ...
                  {isHovered && dayDates.length > 0 && (
                    <div className="absolute z-50 bg-white border border-gray-200 rounded-lg shadow-lg p-3 bottom-full left-0 mb-2 min-w-64">
                      <div className="font-semibold text-sm mb-2">{format(day, "MMM d, yyyy")}</div>
      ...
      <Sheet open={showDayDetails} onOpenChange={setShowDayDetails}>
        <SheetContent className="clay-element border-0 w-full max-w-lg">
          <SheetHeader>
            <SheetTitle className="text-2xl">{selectedDay && format(selectedDay, "MMMM d, yyyy")}</SheetTitle>
          </SheetHeader>
          ...
          <div className="py-6 space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                {selectedDay &&
                  getDatesForDay(selectedDay).map((dateItem) => (
                    <div key={dateItem.id} className="clay-element p-3 rounded-lg">
                      ...
                      {dateItem.notes && <p className="text-sm text-gray-500 mt-2">{dateItem.notes}</p>}
                    </div>
                  ))}
