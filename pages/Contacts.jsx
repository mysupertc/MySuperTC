...
import { Client, Contact } from '@/api/entities';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Loader2, Search, User, Briefcase, Building, Users } from 'lucide-react';

const CONTACT_GROUPS = [
  { title: 'Buyer/Seller', key: 'client', icon: User, types: ['seller', 'buyer'] },
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {contactsInGroup.map(contact => (
                  <div key={contact.id} className="clay-element p-4">
                    ...
                    {contact.transaction_id && <p className="text-xs text-gray-400 mt-2">From Transaction: {contact.transaction_id.slice(0, 8)}...</p>}
                  </div>
                ))}
              </CardContent>
            </Card>
