...
const STATUS_GROUPS = [
  { title: "Pre-Listing", key: "pre_listing" },
...
export default function Pipeline() {
  ...
  const loadTransactions = useCallback(async () => {
    setLoading(true);
    try {
      const pipelineStatuses = ["pre_listing", "listed", "prospecting"];
      const data = await Transaction.filter({
        status: { $in: pipelineStatuses }
      });
      setTransactions(data);
    } catch (error) {
      console.error('Error loading pipeline transactions:', error);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your upcoming deals and listings.</p>
        </div>
      ...
      {loading ? (
        <div className="flex justify-center items-center h-64">
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No pipeline deals yet</h3>
            <p className="text-gray-500 mb-6">Create a new transaction to start building your pipeline.</p>
            <Link to={createPageUrl("NewTransaction")}>
