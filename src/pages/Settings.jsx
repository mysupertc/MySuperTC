...
import { DisclosureTemplate } from '@/api/entities';
...
import { startGmailSync } from "@/api/functions";
import { stopGmailSync } from "@/api/functions";
import { getGoogleCallbackUrl } from '@/api/functions';
...
export default function SettingsPage() {
  ...
  const handleConnectGmail = async () => {
    try {
      const { getGoogleAuthUrl } = await import('@/api/functions');
  ...
  const handleDisconnectGmail = async () => {
    if (!confirm('Are you sure you want to disconnect Gmail? You will no longer be able to send/receive emails through your Gmail account.')) {
      return;
    }
    ...
    try {
      await User.updateMyUserData({
        ...
        is_gmail_sync_enabled: false // Also disable sync when disconnecting
      });
      setUser(prev => ({ ...prev, is_gmail_connected: false, google_email: null, is_gmail_sync_enabled: false }));
    } catch (error) {
  ...
  const handleSyncToggle = async () => {
      ...
      const previousSyncState = user.is_gmail_sync_enabled; // Store the current state
      setSyncing(true);

      try {
          if (previousSyncState) { // If it was previously enabled, try to disable it
              await stopGmailSync();
              setUser(prev => ({ ...prev, is_gmail_sync_enabled: false }));
          } else { // If it was previously disabled, try to enable it
              await startGmailSync();
              setUser(prev => ({ ...prev, is_gmail_sync_enabled: true }));
          }
      } catch (error) {
          ...
          // Revert UI state on failure to its original state
          setUser(prev => ({ ...prev, is_gmail_sync_enabled: previousSyncState })); 
      } finally {
          setSyncing(false);
      }
  };
  ...
  const disclosureColumns = [
    { title: "Document Name", key: "document_name", type: "text" },
    { title: "Notes", key: "notes", type: "text" },
    { title: "N/A S/B", key: "no_seller_buyer", type: "checkbox" },
  ];

  const taskColumns = [
    { title: "Section", key: "section", type: "select", options: ["agent_broker", "escrow_title"] },
    { title: "Task Name", key: "task_name", type: "text" },
    { title: "Order", key: "order_index", type: "number" },
  ];

  // Define columns for Email Templates
  const emailTemplateColumns = [
    { title: "Template Name", key: "name", type: "text" },
    { title: "Subject", key: "subject", type: "text" },
    { title: "Body", key: "body", type: "richtext" },
    { title: "Category", key: "category", type: "select", options: ["General", "Transaction", "Marketing", "Follow-up", "Custom"] },
  ];
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-8">
      ...
      <Tabs defaultValue="appearance" className="w-full">
        <TabsList className="grid w-full grid-cols-4 clay-element max-w-2xl">
          ...
          <TabsTrigger value="disclosures"><List className="w-4 h-4 mr-2" />Disclosures</TabsTrigger>
        ...
        <TabsContent value="email">
          ...
          <Card className="clay-element mt-4">
            <CardHeader>
              ...
              <CardDescription>Connect your Gmail account to send and receive emails directly from the app.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {user?.is_gmail_connected ? (
                <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg border border-green-200">
                  ...
                  <Button variant="outline" onClick={handleDisconnectGmail} className="text-red-600 border-red-200 hover:bg-red-50">
                    Disconnect
                  </Button>
                </div>
              ) : (
               ...
               {user?.is_gmail_connected && (
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  >
                    {syncing ? <Loader2 className="w-4 h-4 animate-spin" /> : (user?.is_gmail_sync_enabled ? "On" : "Off")}
                  </Toggle>
                </div>
              )}
              ...
              <div className="mt-6 p-4 rounded-lg border bg-gray-50">
                  ...
                  <div className="text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => { navigator.clipboard.writeText(oauthInfo.callbackUrl); }}
                        className="h-8"
                  ...
                  <div className="text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => { navigator.clipboard.writeText(oauthInfo.jsOrigin); }}
                        className="h-8"
                ...
                <p className="text-xs text-gray-500 mt-3">
                  Paste these values into Google Cloud Console → APIs & Services → Credentials → OAuth 2.0 Client → Edit.
                </p>
              </div>
            </CardContent>
          </Card>
