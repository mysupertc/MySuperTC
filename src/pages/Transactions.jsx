...
export default function Transactions() {
  ...
  // Function to geocode transactions missing coordinates
  const geocodeMissingTransactions = useCallback(async (transactionList) => {
    const missingCoords = transactionList.filter(t => 
      t.property_address && (!t.latitude || !t.longitude)
    );

    if (missingCoords.length > 0) {
      console.log(`Auto-geocoding ${missingCoords.length} transactions...`);
      
      // Process geocoding with small delays to avoid rate limiting
      for (const transaction of missingCoords) {
        try {
          ...
          console.error(`Failed to geocode ${transaction.property_address}:`, error);
        }
      }
      
      // Reload transactions after geocoding
      const updatedData = await Transaction.list('-created_date');
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [allData, tokenResponse] = await Promise.all([
      ...
      if (tokenResponse.data && tokenResponse.data.token) {
        ...
        console.error("Failed to fetch Mapbox token or token is empty:", tokenResponse);
      }

      // Auto-geocode transactions missing coordinates
      geocodeMissingTransactions(allData);

    } catch (error) {
      console.error('Error loading transactions or map token:', error);
    } finally {
      setLoading(false);
    }
  }, [geocodeMissingTransactions]);
  ...
  const groupedTransactions = useMemo(() => {
    ...
    // Filter transactions for display in groups
