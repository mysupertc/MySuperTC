...
import { Plus, Trash2, User, Building, Briefcase, Search, UserPlus } from 'lucide-react';
import { Contact, Client } from '@/api/entities';
...
const CONTACT_TYPES = {
  ...
  seller_agent: { label: 'Seller\'s Agent', icon: Briefcase, fields: ['name', 'dre_number', 'email', 'cell_phone', 'office_phone', 'address', 'website', 'notes'] },
  seller_brokerage: { label: 'Seller\'s Brokerage', icon: Building, fields: ['name', 'dre_number', 'email', 'cell_phone', 'office_phone', 'address', 'website', 'notes'] },
  buyer_agent: { label: 'Buyer\'s Agent', icon: Briefcase, fields: ['name', 'dre_number', 'email', 'cell_phone', 'office_phone', 'address', 'website', 'notes'] },
  buyer_brokerage: { label: 'Buyer\'s Brokerage', icon: Building, fields: ['name', 'dre_number', 'email', 'cell_phone', 'office_phone', 'address', 'website', 'notes'] },
  escrow: { label: 'Escrow', icon: Building, fields: ['name', 'email', 'team_email', 'cell_phone', 'office_phone', 'address', 'website'] },
  title: { label: 'Title', icon: Building, fields: ['name', 'email', 'team_email', 'cell_phone', 'office_phone', 'address', 'website'] },
  lender: { label: 'Lender', icon: Building, fields: ['name', 'email', 'team_email', 'cell_phone', 'office_phone', 'address', 'website'] },
  misc: { label: 'Misc.', icon: User, fields: ['name', 'email', 'cell_phone', 'notes'] },
};

const FIELD_LABELS = {
  name: "Name", dre_number: "DRE#", email: "Email", team_email: "Team Email", cell_phone: "C: Phone", office_phone: "O: Phone", address: "Address", website: "Website", notes: "Notes"
};
...
export default function Contacts({ contacts, transactionId, onUpdate, isCreationMode = false, onContactsChange }) {
  ...
  const loadGlobalContacts = async () => {
    try {
      const [allContacts, allClients] = await Promise.all([
        Contact.list(),
        Client.list()
      ]);
  ...
  const handleUpdateContact = useCallback(async (contact, field, value) => {
    if (isCreationMode) {
              ...
              <div className="space-y-4">
                {sectionContacts.map(contact => (
                  <div key={contact.id} className="clay-element p-4 rounded-lg space-y-2 relative group">
                    <Button variant="ghost" size="icon" className="absolute top-2 right-2 opacity-0 group-hover:opacity-100" onClick={() => handleDeleteContact(contact.id)}>
                ...
                {isAdding && newContact.contact_type === typeKey && (
                  <div className="clay-element p-4 rounded-lg space-y-2">
                    ...
                    <div className="flex gap-2 justify-end pt-2">
                      <Button onClick={handleAddNewContact} size="sm" className="clay-element clay-accent-mint border-0">Save</Button>
                      <Button variant="ghost" size="sm" onClick={() => setIsAdding(false)} className="clay-element border-0">Cancel</Button>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="clay-element p-4 rounded-lg space-y-4">
            ...
            <Select value={newContact.contact_type} onValueChange={(v) => setNewContact({ contact_type: v })}>
              ...
              <SelectContent className="clay-element border-0">
                {Object.entries(CONTACT_TYPES).map(([key, val]) => <SelectItem key={key} value={key}>{val.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
        )}
